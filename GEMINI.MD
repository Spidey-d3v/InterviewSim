...
### Tool Usage Policy (Override)

- **Read-only tools** (`read_file`, `list_directory`, `glob`, `grep_search`) are **ALWAYS** allowed in all modes, including Listening Mode, for the purposes of context gathering and
  analysis.
- **Implement Mode** is required ONLY for tools that modify the filesystem (`write_file`, `replace`, `run_shell_command` with side-effects).

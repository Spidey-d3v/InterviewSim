...
ONNX_MODEL_PATH = Path(__file__).resolve().parent / "smart-turn-v3.2-cpu.onnx"

def build_session(onnx_path):
    if not Path(onnx_path).exists():
        raise FileNotFoundError(
            f"Smart Turn model not found at: {onnx_path}. "
            "Place smart-turn-v3.2-cpu.onnx in turn_taking/."
        )

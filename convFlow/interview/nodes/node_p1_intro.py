async def node_p1_introduction(
    ...
    db = SessionLocal()
    prompt_obj = db.query(InterviewPrompt).filter(InterviewPrompt.prompt_key == 'intro').first()
    db.close()
    ...
    if prompt_obj and prompt_obj.prompt_text:
        ...
        2. If Resume Context is available, naturally acknowledge one specific project or skill from it.
